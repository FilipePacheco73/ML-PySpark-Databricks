...
file_path = file_path = "xxxxxxxxxxxxxxxxxx"
...
vec_assembler = VectorAssembler(inputCols=['bedrooms','bathrooms','bathrooms_na','minimum_nights','number_of_reviews'], outputCol='features') # FILL_IN

vec_train_df = vec_assembler.transform(train_df)
vec_test_df = vec_assembler.transform(test_df)

lr_model = LinearRegression(labelCol='price').fit(vec_train_df) # FILL_IN

pred_df = lr_model.transform(vec_test_df) # FILL_IN

regression_evaluator = RegressionEvaluator(predictionCol="prediction", labelCol="price", metricName="rmse")
rmse = regression_evaluator.evaluate(pred_df) # FILL_IN
r2 = regression_evaluator.setMetricName('r2').evaluate(pred_df) # FILL_IN
