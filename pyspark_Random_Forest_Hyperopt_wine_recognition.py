...
train_df, val_df, test_df = df_spark.randomSplit([.6, .2, .2], seed=42)
...
rf = RandomForestRegressor(labelCol='target', maxBins=80, seed=42)
pipeline = Pipeline(stages=[string_indexer, vec_assembler, rf])
regression_evaluator = RegressionEvaluator(predictionCol='prediction', labelCol='target')
